                ...
                <div class="card no-b m-2">
                    <div class="card-body">
                        <div class="mt-2 mb-3" style="float: right;">
                            <a href="{{route('MasterDataPengolahan.timbulan_sampah.create')}}" class="btn btn-primary btn-sm"> <i class="icon icon-plus white-text s-18"></i>Timbulan Sampah</a>
                        </div>
                        <div class="table-responsive">
                            <table id="dataTable" class="table table-striped table-bordered" style="width:100%">
                                <thead>
                                    ...
                                    <th>Timbulan Sampah Tahunan (ton)</th>
                                    <th width="60">Aksi</th>
                                </thead>
                                <tbody></tbody>
                            </table>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        var table = $('#dataTable').dataTable({
            ...
            order: [],
            ajax: {
                url: "{{ route('MasterDataPengolahan.timbulan_sampah.api') }}",
                method: 'POST',
                data: function(data){
            ...
            columns: [
                ...
                {data: 'timbul_harian', name: 'timbul_harian', className: 'text-center'},
                {data: 'timbul_tahunan', name: 'timbul_tahunan', className: 'text-center'},
        ...
        $.confirm({
            ...
            buttons: {
                ok: {
                    ...
                    keys: ['enter'],
                    action: function(){
                        $.post("{{ route('MasterDataPengolahan.timbulan_sampah.destroy', ':id') }}".replace(':id', id), {'_method' : 'DELETE'}, function(data) {
                            table.api().ajax.reload();
                            if(id == $('#id').val()) add();
                        }, "JSON").fail(function(){
                            location.reload();
